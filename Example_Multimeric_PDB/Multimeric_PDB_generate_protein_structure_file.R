...
#Load xml file 
xmlfile <- xmlParse("Example_Multimeric_PDB/Example_input/6d6t.xml.gz") 

xmllist <- xmlToList(xmlfile)

#Select pdb ID and desired ALL uniprot IDs from protein of interest. Nessesary if the PDb file contains several proteins 
uniprot <- c("P14867","P47870","P18507")
genes <- c("GABRA1","GABRB2","GABRG2")


uniprot_gene.df <- tibble(uniprot = uniprot,genes = genes)

pdbid <- "6d6t" #GABA receptor 
...
for (match in 1:(length(entity_uniprot_matches))){
  ...
  if(entity_uniprot_matches[match] != 0 & length(which(is.na(entity_uniprot_matches))) == 0){
    ...
    uniprot <- iterate_df$uniprot[match]
    gene <- uniprot_gene.df$genes[which(uniprot == uniprot_gene.df$uniprot)]
    
    print(gene)
    
    fasta_file <- read.fasta(paste0("Example_Multimeric_PDB/Example_input/",gene,".fasta"))
    ...
    ##check if Paraz score is avalable for the right transcript 
    
    paraz <- read_delim(paste0("Example_Multimeric_PDB/Example_input/",gene,"_Paraz.txt"),delim = "\t",skip = 11) %>% .$`(SCORE-MEAN)/STD`
    
    if(length(paraz) ==nrow(data)){
    ...
    ##check if mtr score is avalable for the right transcript 
    mtr <- data$MTR <- read_delim(paste0("Example_Multimeric_PDB/Example_input/",gene,"_MTR.txt"), delim = "\t") %>% .$mtr
    
    if(length(mtr) ==nrow(data)){
    ...
    write.table(data,paste0("Example_Multimeric_PDB/Example_output/Sequence_to_structure/6d6t_",gene,"_chain_",chain,"_segment",segment,".txt"), sep = "\t", dec =".",row.names = F)
