...
density_test_combined <- function(file_patient, file_control, file_path, radius){### calculate the scores from scratch, file_type and file_path define protein files which should be loaded. In this example the experimental input files i have sent you were taken (protein: DDX3X)
...
#add variants to protein file


variants.df <- read_delim("Example_Multimeric_PDB/Example_input/6d6t_example_variant_input.txt", delim  = "\t")

protein_w_control.df <- read_delim("Example_Multimeric_PDB/Example_output/6d6t_protein_structure_score_input.txt", delim  = "\t") %>% 
  left_join(variants.df %>% select(-patient_variants), by = c("gene" = "gene","Uniprot_position" = "Aminoacid_position","aminoacid" = "ref_aa")) %>% 
  rename(variants = "control_variants")

protein_w_patient.df <- read_delim("Example_Multimeric_PDB/Example_output/6d6t_protein_structure_score_input.txt", delim  = "\t") %>% 
  left_join(variants.df %>% select(-control_variants), by = c("gene" = "gene","Uniprot_position" = "Aminoacid_position","aminoacid" = "ref_aa")) %>% 
  rename(variants = "patient_variants")


density_test_combined(protein_w_patient.df,protein_w_control.df,file_path_expi,radius) %>% 
  ...
  write_delim("Example_Multimeric_PDB/Example_output/6d6t_3D_scores.txt", delim = "\t")
