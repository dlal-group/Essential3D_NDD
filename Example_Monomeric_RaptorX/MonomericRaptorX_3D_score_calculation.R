...
variants.df <- read_delim("Example_Monomeric_RaptorX/Example_input/SLC2A1_example_variant_input.txt", delim  = "\t")

protein_w_control.df <- read_delim("Example_Monomeric_RaptorX/Example_output/SLC2A1_protein_structure_chain_A_score_input.txt", delim  = "\t") %>% 
  mutate(variants = variants.df$control_variants)

protein_w_patient.df <- read_delim("Example_Monomeric_RaptorX/Example_output/SLC2A1_protein_structure_chain_A_score_input.txt", delim  = "\t") %>% 
  mutate(variants = variants.df$patient_variants)


density_test_combined(protein_w_patient.df,protein_w_control.df,radius) %>% 
  mmutate(PER3D = ifelse(OR >1 & pvalue < 0.05,"PER3D","No-PER3D"),
  ...
  write_delim("Example_Monomeric_RaptorX/Example_output/SLC2A1_3D_score_output.txt", delim = "\t")
